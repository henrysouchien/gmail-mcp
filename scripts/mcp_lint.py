...
MCP_PATH_RE = re.compile(
    r"(^|/)mcp_server\.py$|(^|/)x_mcp_server\.py$|(^|/)mcp_servers/.*\.py$|(^|/)server\.py$"
)
...
def _find_tools(tree: ast.AST) -> list[ToolFunction]:
    ...
    for node in ast.walk(tree):
        if not isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            continue
        decorated = any(is_mcp_tool(decorator) for decorator in node.decorator_list)
...
def _lint_tool(path: Path, tool: ToolFunction, sibling_names: set[str]) -> list[Issue]:
    ...
    if DESTRUCTIVE_VERB_RE.search(tool.name) and not (params & SAFETY_PARAMS):
        add(
            "error",
            "L-002",
    ...
    if _returns_bare_string(func) and _raises_generic_error(func):
        add(
            "warn",
            "L-006",
    ...
    if _looks_mutating(tool.name, func) and not _has_try_envelope(func):
        add(
            "warn",
            "L-007",
